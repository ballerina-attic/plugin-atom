...
currentPanelId = -1;
listener = new DOMListener(document.querySelector('.line-numbers'))
...
class LanguageBallerina {
  ...
  activate() {
    ...
    this.provider = new AtomAutocompleteProvider(this.classLoader);
      ...
      console.log("error listener inititlizing");
          ...
          return function(ecognizer, offendingSymbol, line, column, msg, e) {
              line--;
          		...
          		if (editor = atom.workspace.getActiveTextEditor()) {
          			 ...
          			 let disposable;
          			 listener.add('div.line-number.red-square.id' + markerCounter + '-' + currentPanelId, 'mouseover', function(event) {
          			 ...
          			 listener.add('div.line-number.red-square.id' + markerCounter + '-' + currentPanelId, 'mouseout', function(event) {
          				 disposable.dispose();
          			 });
