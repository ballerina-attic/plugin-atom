...
class LanguageBallerina {
    ...
    activate() {
        ...
        this.subscriptions.add(atom.commands.add('atom-workspace', {
            'language-ballerina:parse': () => parse()
        }))
        // Listen for pane change
        this.subscriptions.add(
            atom.workspace.onDidStopChangingActivePaneItem(function(paneItem) {
                onChange(paneItem);
            })
        );
        ...
        atom.workspace.observeTextEditors(function (editor) {
            ...
            return editorView.addEventListener('keyup', parse);
        });
        ...
        this.classLoader.loadProjectDependencies(atom.project.getDirectories()[0].realPath + "/");
    }

    _onSave(editor) {
        var filePath = editor.buffer.file.path;
        var fileFolder = atomBallerinaUtil.getPackageName(filePath);
        var fileName = atomBallerinaUtil.getFileName(filePath);
        this.classLoader._addClass(false, editor.getText(), fileFolder, fileName);
    }
